  ...
  return (
    <div className="relative">
      <select
        ...
        disabled={isUpdating}
        className={`text-xs font-semibold rounded-full px-3 py-1 border focus:ring-2 focus:ring-red-500 cursor-pointer transition-colors ${
      ...
      {isUpdating && (
        <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75 rounded-full">
          <div className="w-3 h-3 border border-cyan-600 border-t-transparent rounded-full animate-spin"></div>
        </div>
      )}
    </div>
  );
