  ...
  try {
    ...
    if (!rateCheck.allowed) {
      return NextResponse.json(
        {
          success: false,
          message: `Too many requests. Try again in ${rateCheck.retryAfterMinutes} minutes.`,
        },
        { status: 429 },
      );
    }
    ...
    const { subject, message, statusFilter, targetEmail } =
      await request.json();
    ...
    if (targetEmail) {
      const entry = await Appointment.findOne({
      ...
      if (!entry) {
        return NextResponse.json(
          {
            success: false,
            message: "Email address not found in appointments",
          },
          { status: 404 },
        );
      }
      ...
      recipientEmails = [
        ...new Set(appointments.map((a) => a.email.toLowerCase())),
      ];
    }
