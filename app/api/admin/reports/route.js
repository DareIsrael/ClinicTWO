...
export async function GET(request) {
  try {
    ...
    const statusCounts = usersByStatus.reduce(
      (acc, item) => {
    ...
    const appointmentStatusCounts = appointmentsByStatus.reduce(
      (acc, item) => {
    ...
    return NextResponse.json({
      ...
      report: {
        ...
        summary: {
          ...
          acceptanceRate:
            newPatients > 0
              ? Math.round(((statusCounts.Accepted || 0) / newPatients) * 100)
              : 0,
          rejectionRate:
            newPatients > 0
              ? Math.round(((statusCounts.Rejected || 0) / newPatients) * 100)
              : 0,
        },
      },
    });
